...
msdf_signedDistance msdf_quadraticDist(msdf_EdgeSegment *e, msdf_Vec2 origin, double *param)
{
    ...
    *param = -msdf_v2MulInner(qa, ab) / msdf_v2MulInner(ab, ab);
    ...
    if (*param < .5)
        return {min_distance, fabs(msdf_v2MulInner(ab, qa))};
    else
        return {min_distance, fabs(msdf_v2MulInner(aa, bb))};
}
...
msdf_signedDistance msdf_cubicDist(msdf_EdgeSegment *e, msdf_Vec2 origin, double *param)
{
    ...
    *param = -msdf_v2MulInner(qa, ep_dir) / msdf_v2MulInner(ep_dir, ep_dir);
    ...
    if (*param < .5)
        return {min_distance, fabs(msdf_v2MulInner(d0, qa))};
    else
        return {min_distance, fabs(msdf_v2MulInner(d1, a))};
}
...
int msdf_genGlyph(msdf_Result* result, stbtt_fontinfo *font, int stbttGlyphIndex, uint32_t borderWidth, float scale, float range, msdf_AllocCtx* alloc) {
    ...
    for (int i = 0; i < contour_count; ++i) {
        ...
        int contourCornerStart = cornerIndex;
        ...
        int contourCornerCount = cornerIndex - contourCornerStart;

        if (contourCornerCount == 0) {
            for (int j = 0; j < contour_data[i].edge_count; ++j) {
                contour_data[i].edges[j].color = msdf_edgeColor_white;
            }
        } else if (contourCornerCount == 1) {
            ...
            int corner = corners[contourCornerStart];
            ...
            int spline = 0;
            int start = corners[contourCornerStart];
            int m = contour_data[i].edge_count;
            ...
            for (int j = 0; j < m; ++j) {
                int index = (start + j) % m;
                if (spline + 1 < contourCornerCount && corners[contourCornerStart + spline + 1] == index) {
                    ++spline;

                    msdf_edgeColor s = (msdf_edgeColor)((spline == contourCornerCount - 1) * initial_color);
